...
removals (2026-09-07) it carries the 461 entries
...
def test_candidate_signatures_accepted_by_v2_loader():
    """The tracked signatures candidate is a valid v2 document with 461 entries.

    Derivation of the 461 pin: the candidate is the activation artifact that
    ...
    471 post-GR-14h, 461 post-GR-14u (dead-writer tranche 2),
    ...
    regenerating the tracked pair at 471 (GR-14u: 461); promotion realigns active.  The earlier PR-GR-05 truth
    ...
    assert len(document) == 461, (
        "candidate must carry exactly the 461 current signature entries"
    )
