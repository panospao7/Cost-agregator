...
55 unique canonical mutation keys resolved from the 99 legacy v1 inputs
...
def test_candidate_signatures_accepted_by_v2_loader():
    """The PR-GR-05 signatures candidate is a valid v2 document with 55 entries.

    The tracked candidate carries exactly one entry per unique canonical
    mutation key resolved from the 99 legacy v1 inputs (55 keys after the
    Slice 4/5 dedupe fold; 44 indices remain explicit debt rows).  Byte
    ...
    assert len(document) == 55, (
        "candidate must carry exactly the 55 resolved signature entries"
    )
