...
v2 document (``schemaVersion: 2``, 9 entries) regenerated byte-for-byte by
``scripts/migrate_db_policy_signatures.py``.  The v2 loader must accept it
with zero errors.
...
def test_candidate_signatures_accepted_by_v2_loader():
    """The GR-02 signatures candidate is a valid v2 document with 9 entries."""
    document, errors = _load_or_skip(CANDIDATE_SIGNATURES_PATH)
    assert document is not None, (
        "GR-02 signatures candidate must be accepted by the v2 loader"
    )
    assert len(document) == 9, (
        "candidate must carry exactly the 9 resolved signature entries"
    )
