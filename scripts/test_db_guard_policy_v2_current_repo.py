...
removals (2026-09-07) it carries the 434 entries
...
def test_candidate_signatures_accepted_by_v2_loader():
    """The tracked signatures candidate is a valid v2 document with 434 entries.

    Derivation of the 434 pin: the candidate is the activation artifact that
    ...
    471 post-GR-14h, 461 post-GR-14u, 451 post-GR-14u2, 441 post-GR-14u3, 434 post-GR-14u4 (dead-writer tranche 5),
    ...
    regenerating the tracked pair at 471 (GR-14u: 461; GR-14u2: 451; GR-14u3: 441; GR-14u4: 434); promotion realigns active.  The earlier PR-GR-05 truth
    ...
    assert len(document) == 434, (
        "candidate must carry exactly the 434 current signature entries"
    )
