#!/usr/bin/env python3
"""
Static money-boundary guard v4 for ExpenseTracker.

CURR-587-07 / CURR-587-finalization: Hardened guard with call-block scanner,
structured allowlists, and all missing rules implemented.
...
Rules:
  ...
  G-MONEY-08  No raw sums (sumOf { it.amount }, sumOf { it.effectiveAmount })
  ...
  G-MONEY-11  No fake unavailable currency sentinels CurrencyCode("XXX") or CurrencyCode("") (NON-ALLOWLISTABLE)
...
def extract_call_block(content: str, start_index: int) -> str:
    """Extract a balanced parenthesis block starting at start_index."""
    depth = 0
    i = start_index
    in_string = False
    escape = False
...
def find_constructor_blocks(content: str, name: str) -> List[Tuple[int, str]]:
    """Find all call blocks for a given constructor/function name."""
    blocks = []
    pattern = re.compile(rf'\b{re.escape(name)}\s*\(')
    for m in pattern.finditer(content):
        block = extract_call_block(content, m.start())
        line_num = content[:m.start()].count('\n') + 1
        blocks.append((line_num, block))
    return blocks
...
RULES: List[Rule] = [
    ...
    # G-MONEY-15: dashboard raw money — line-level detection
    Rule(r'summary\.totalSpent', 'G-MONEY-15',
         'Dashboard widget must not use raw summary.totalSpent — use normalized input',
         financial_only=True),
    Rule(r'weather\.discretionaryBudget', 'G-MONEY-15',
         'Dashboard widget must not use weather.discretionaryBudget as money — use normalized input',
         financial_only=True),
    Rule(r'getHomeCurrencyPurchaseTotal\(', 'G-MONEY-15',
         'Dashboard must not call latest-rate getHomeCurrencyPurchaseTotal — use normalized input',
         financial_only=True),
    Rule(r'getHomeCurrencyDepositTotal\(', 'G-MONEY-15',
         'Dashboard must not call latest-rate getHomeCurrencyDepositTotal — use normalized input',
         financial_only=True),

    # G-MONEY-16: SpendingTrend direct conversion — NON-ALLOWLISTABLE
...
# ── G-MONEY-13: BudgetForecast unavailable sentinel (call-block based) ─────────
BUDGET_FORECAST_UNAVAILABLE_SENTINEL_RE = re.compile(
    r'HomeCurrencyResolution\.Failed[\s\S]{0,600}?BudgetForecast\s*\(',
    re.MULTILINE
)
BUDGET_FORECAST_SAFE_RE = re.compile(
    r'BudgetForecastResult\.Unavailable|ForecastCurrencyStatus\.HOME_CURRENCY_UNAVAILABLE|ForecastCurrencyStatus\.LIMIT_CONVERSION_FAILED'
)
...
# G-MONEY-15 is only relevant inside the dashboard use case file
G_MONEY_15_FILE_PATTERN = re.compile(r'ComputeDashboardWidgetsUseCase', re.IGNORECASE)
...
def check_file(file_path: Path) -> List[Violation]:
    ...
    is_dashboard_usecase = G_MONEY_15_FILE_PATTERN.search(file_path.name) is not None

    # G-MONEY-13: BudgetForecast unavailable sentinel (multiline call-block)
    if 'budget' in path_str.lower():
        for match in BUDGET_FORECAST_UNAVAILABLE_SENTINEL_RE.finditer(content):
            block = match.group()
            if not BUDGET_FORECAST_SAFE_RE.search(block):
                line_num = content[:match.start()].count('\n') + 1
                violations.append(Violation(
                    line_num, 'G-MONEY-13',
                    'Normal BudgetForecast returned in unavailable home-currency branch — use BudgetForecastResult.Unavailable',
                    block[:120].replace('\n', ' ')
                ))
    ...
    for line_idx, line in enumerate(lines):
        ...
        for rule in RULES:
            if rule.financial_only and not file_is_financial:
                continue
            # G-MONEY-15 only applies inside dashboard use case
            if rule.rule_id == 'G-MONEY-15' and not is_dashboard_usecase:
                continue
            # G-MONEY-16 only applies inside dashboard use case
            if rule.rule_id == 'G-MONEY-16' and not is_dashboard_usecase:
                continue
...
def main():
    parser = argparse.ArgumentParser(description='Static money-boundary guard v4')
    ...
    if total_violations == 0:
        ...
        print("\nNon-allowlistable rules (G-MONEY-10/11/12/13/16) cannot be suppressed.")
        sys.exit(1)
