---
name: strict-imported-plan
description: Execute an external review/debug doc plus implementation plan one strict batch at a time. Use when the user provides a plan from another system to implement carefully.
---

# Strict Imported Plan

Execute an external review/debug document plus implementation plan one strict batch at a time. The review doc, plan, and requested batch come from the invoking message (file paths, pasted context, or instructions). Codex skills have no argument passing: treat everything in the invoking message as that input.
...
5. First ask the `scout` subagent to verify the plan against current source.
6. If the plan is stale, conflicting, or unsafe, stop and report; propose a delta plan.
