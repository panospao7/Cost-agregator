...
├── ApplicationScope.kt                 (Custom scope qualifier — not a @Module)
├── NetworkQualifiers.kt                (Custom network qualifiers)
│
├── DispatchersModule.kt                (Coroutine dispatchers, ApplicationScope)
├── DatabaseModule.kt                   (Room DB: AppDatabase, GroupTransactionCoordinator)
├── DaoModule.kt                        (58 DAO singleton provisions)
├── WorkerModule.kt                     (WorkerRunLogger binding)
├── TimeModule.kt                       (TimeProvider → SystemTimeProvider)
├── ServiceModule.kt                    (Gson, NotificationService, Geocoding, Location, Widget, Speech)
├── NetworkModule.kt                    (OkHttp clients: @LocationHttp, @CloudAiHttp)
├── SecurityModule.kt                   (SecureKeyStorage)
│
├── AiModule.kt                         (AI repos, services, DAOs, RedactionSanitizer, AiPolicy)
├── OcrImprovementsModule.kt            (OCR pipeline: EnhancedMerchantExtractor, OcrLanguageProcessor)
├── NaturalLanguageModule.kt            (NaturalLanguageExpenseQueryRepository)
│
├── CashFlowModule.kt                   (CashFlowCalculator)
├── CurrencyModule.kt                   (CurrencySettingsRepository, ExchangeRates)
├── DashboardContractsModule.kt         (7 dashboard contract adapters)
├── DashboardAnomalyModule.kt           (AnomalyAlertRepository domain + dashboard)
├── DiagnosticsModule.kt                (Diagnostic writers, lifecycle event writers, OperationRunRecorder)
├── SavingsModule.kt                    (SmartSavingsEngine, AutomatedSavingsRule*, Gamification)
├── SavingsRepositoryBindingsModule.kt  (DomainSavingsGoalRepository binding)
├── GroupsModule.kt                     (GroupsRepository, SharedExpenseDataPort, use cases)
├── TaxModule.kt                        (TaxConfiguration → GreeceTaxConfiguration)
├── ExportModule.kt                     (QuickBooksIIF, XeroCSV, FreshBooks exporters)
├── ReminderSettingsModule.kt           (BillReminderSettingsRepository) — P4
├── BackupRepositoryModule.kt           (DatabaseBackupRepository → impl)
├── ParserModule.kt                     (GreekBankParser with injected homeCurrency)
├── ReceiptParsingModule.kt             (MerchantRulesPolicy binding)
├── EmailIngestionModule.kt             (Amazon, Uber, Apple receipt parsers)
├── LocationResolverPortsModule.kt      (LocationCachePort, MerchantClusterPort)
├── EmptyStateModule.kt                 (Empty state registry multibind)
├── PrivacyModule.kt                    (CompositePrivacyGate, PrivacyAuditLogger, PrivacySettings)
├── ProvenanceModule.kt                 (Provenance event recording)
├── RetentionModule.kt                  (RetentionRegistry with 5 targets)
└── EmptyStateRegistryInitializer.kt    (Registry bootstrap)
```
📝 **Note:** `AppModule.kt` (placeholder) and `InvestmentModule.kt` removed; `SubscriptionModule.kt` deleted in 2026-05-09 refactoring. See `docs/architecture/hilt-bindings-map.md` for full binding details (31 @Module files).
