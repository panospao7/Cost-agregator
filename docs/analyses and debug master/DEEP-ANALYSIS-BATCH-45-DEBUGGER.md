...
| 4 | WarrantyTextExtractor.kt | **MAJOR** | Thread Safety | Warranty extraction uses shared `SimpleDateFormat` instances from a singleton pipeline component, which is not thread-safe under parallel receipt batch processing. | 1. Process 10 receipts in parallel. 2. Shared `SimpleDateFormat` produces corrupted dates. 3. Wrong warranty dates stored. | Replace with thread-safe `java.time` formatters or create formatter instances per call. |
