...
**VERDICT: FAIL for unconditional merge readiness; no confirmed source regression.** WI-1, WI-2 and WI-3 are DONE-CORRECT. WI-4's implementation and corrected assertions are substantively correct, but its required pre-edit A8 disposition/guardian evidence is not established by the reviewed repository records: **PARTIAL**. The separately authorized forecast-constraint deviation is **SANCTIONED**, for the current production call path and schema.

This is not a claim that the human never approved A8: it is a distinction between verified implementation/execution evidence and missing approval provenance. The review request explicitly authorizes the constraint-outcome deviation; it does not establish when the separate A8 assertion decision and required guardian reviews occurred.
...
| WI-4 — reconcile A8 assertions | **PARTIAL — approval provenance outstanding, code/tests correct** | `T/scenarios/MoneyAggregateConversionScenarioTest.kt:240–241,355–359,437–456,519–525`; `T/domain/budget/BudgetForecastingEngineTest.kt:988–1018`; real-producer regressions at `T/domain/core/money/MoneyAggregateBuilderRestrictionTest.kt:126–146` and `CurrencyNormalizationBehavioralTest.kt:141–160`. Correct fixed-contract assertions and 6/6 scenario plus 32/32 forecast execution are verified. However, spec lines 64–68/75 require recorded human disposition and guardian review before expectation edits; that record was not found in the inspected register, journal, workflow evidence or lane commits. See G1. |
...
- No standalone `compile`-profile result was present among the 13 local run records inspected. The successful targeted runs include build/compilation tasks, but this review does not relabel them as the spec's separate final compile-profile run. No full suite, lint or static-guard PASS is asserted.
...
- **[MAJOR-GATE G1] A8 decision provenance is unverified.** The governing spec explicitly requires a before/fixed table, recorded human disposition and room-migration/architecture guardian review before assertion edits (`CL-27-implementation-spec.md:64–68,75,95–97`). The inspected `../cluster-map.md:464–469,511–514` contains placement approvals and the binding A8 mandate, not the required assertion-specific disposition. The journal's CL-27 entry records spec writing, not implementation approval; no matching active-workflow record or approval artifact appears in this lane. Attach the actual prior decision/review evidence, or obtain an explicit human disposition of the missing provenance; do not fabricate a pre-edit approval date. This is why WI-4 remains PARTIAL despite correct code and passing execution.
- **[MAJOR-GATE G2] Required guardian sign-off is not evidenced here.** Source inspection supports no schema/migration/barrier change, but this reviewer does not impersonate room-migration-guardian or architecture-guardian approval. Supply the required existing sign-offs, or have the owner close the gate. No subagents were spawned under this review's hard rules.
- **[MINOR-GATE G3] Separate final compile evidence is absent.** Supply the spec's compile-profile result or an explicit human acceptance that the already-successful targeted builds satisfy that remaining validation step. Only the human/validation-runner owner may run it; this reviewer did not.
- **[MINOR ADVISORY] Constraint branch coverage can be stronger.** An end-to-end unavailable/diagnostic/no-success-event assertion and explicit null/non-UNIQUE constraint cases would protect the sanctioned deviation. Current UNIQUE/FK privacy tests are meaningful, and the untested fallback/dispatch is correct by source inspection; this is not a demonstrated regression.

**Remaining steps to mergeable:** close G1 and G2 with attributable evidence/human disposition, and close G3 with evidence/explicit acceptance. No source correction is requested by this review. A future schema/index change must revisit the deliberately narrow UNIQUE-message classifier.
...
Files touched by the reviewer: only this new `CL-27-review.md`.

Validation by reviewer: **NOT RUN**, as explicitly required. Human execution evidence and its limits are documented above. Commit this artifact alone on `rp-26-wip`; return the resulting SHA and proposed journal line to the human. Do not edit JOURNAL.md.
