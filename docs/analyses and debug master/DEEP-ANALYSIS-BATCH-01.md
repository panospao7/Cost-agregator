...
| 5 | domain/analytics/AdvancedAnalyticsDashboard.kt:160-206 | HIGH | Bug/Performance | `getMonthlyTrend()` issues one repository query per month, uses an end timestamp of `23:59:59` as an exclusive bound, and ignores incoming transfers even though `generateDashboardData()` counts them as income. The chart can disagree with the dashboard header and miss end-of-month transactions. | Aggregate the whole range in one query, use exclusive month-end boundaries (`start of next month`), and apply the same income/spending rules as the top-level totals. |
