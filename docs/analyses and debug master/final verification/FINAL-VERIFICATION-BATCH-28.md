...
> **[RESOLVED BY B.4]** All B.4-scope issues from this batch have been addressed across B.4 micro-batches. See `docs/reviews/REVIEW-B4.md` for targeted validation evidence and waivers.
...
|---|----------|----------|------|-------------|----------------|---------------|
| 1 | `ExpenseRepository.deleteExpense()` → `AnomalyAlertDao.getActiveAlerts()` → `ComputeMoneyRadarUseCase` | High | Orphaned reference | **[RESOLVED BY B.4 — Batch 6]** Deleting an expense does not clean up anomaly alerts, and active alerts are rendered without re-checking parent expense existence. | `data/database/entity/AnomalyAlert.kt`, `data/repository/ExpenseRepository.kt`, `data/database/dao/AnomalyAlertDao.kt`, `domain/usecase/dashboard/ComputeMoneyRadarUseCase.kt` | Enforce FK-backed cleanup or delete related alerts inside the expense-deletion transaction. |
| 2 | `NotificationProcessingPipeline.launchSubscriptionDetection()` → `SubscriptionCandidateDao.insert()` | High | Race condition | **[RESOLVED BY B.4 — Batch 7]** Subscription detection runs asynchronously, pre-checks for a pending candidate, then inserts without any DB uniqueness guarantee, so concurrent jobs can create duplicate pending candidates. | `data/database/entity/SubscriptionCandidate.kt`, `data/database/dao/SubscriptionCandidateDao.kt`, `data/repository/NotificationProcessingPipeline.kt` | Add a DB-enforced pending-candidate uniqueness rule and use transactional upsert semantics. |
| 3 | `BudgetForecastingEngine.generateForecast()` → `BudgetForecastDao.getForecastForDate()` | Medium | State ambiguity | **[RESOLVED BY B.4 — Batch 7]** The engine never retires previous active forecasts, so repeated generations for the same budget period leave multiple active rows. Date-based active lookups can then become nondeterministic if overlapping active rows remain. | `data/database/entity/BudgetForecast.kt`, `data/database/dao/BudgetForecastDao.kt`, `domain/budget/BudgetForecastingEngine.kt` | Deactivate prior active rows transactionally before insert and enforce the active-row invariant at the schema/repository layer. |
| 4 | `FinancialHealthScoreV2.saveToHistory()` → `HealthScoreHistoryDao.getHistoryForPeriod()/insert()/update()` | Medium | Non-atomic upsert | **[RESOLVED BY B.4 — Batch 8]** History persistence depends on a non-unique period key plus a read-then-write sequence, so concurrent recalculations can store multiple snapshots for the same period. | `data/database/entity/HealthScoreHistory.kt`, `data/database/dao/HealthScoreHistoryDao.kt`, `domain/health/FinancialHealthScoreV2.kt` | Add a unique period key and collapse persistence into a single transactional upsert. |
| 5 | `NotificationCaptureService` → `NotificationProcessingPipeline` → `RawNotificationDao.insertOrIgnore()` | High | Dedupe gap | **[RESOLVED BY B.4 — Batch 6]** Partially-null raw notifications can bypass the unique index, so duplicate notifications may be reprocessed and, in the oversized-fallback path, create duplicate pending reviews. | `data/database/entity/RawNotification.kt`, `data/database/dao/RawNotificationDao.kt`, `service/NotificationCaptureService.kt`, `data/repository/NotificationProcessingPipeline.kt` | Normalize nullable dedupe fields before persistence or rebuild uniqueness around coalesced/generated columns. |
...
## Key Patterns
- At the time of verification, business invariants were repeatedly implemented in application code but not enforced in schema (`expenseId` ownership, unique health periods, unique pending subscription candidates, single active forecast semantics). Resolved by B.4 — Batches 6, 7, and 8.
- At the time of verification, several hot DAO paths were missing the indexes they actually queried (`category` in anomaly alerts, `originalMerchant` in user corrections). Both were added by B.4 late closeout: `originalMerchant` index via `MIGRATION_76_77` (schema version 77); `(category, alertedAt)` index via `MIGRATION_77_78` (schema version 78).
- At the time of verification, nullable columns were being used inside uniqueness contracts, which broke dedupe assumptions under SQLite semantics. Resolved by B.4 — Batch 6 (`RawNotification` nullable-field dedupe loophole).
- At the time of verification, manual-entry and analytics tables lacked basic validation guardrails, allowing malformed numeric data to flow into tax and reporting totals. Resolved by B.4 — Batch 8 (`MileageTracking`, `SavingsGoal` check constraints and validation guards).
