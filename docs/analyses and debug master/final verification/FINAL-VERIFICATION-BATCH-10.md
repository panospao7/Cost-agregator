...
| 10 | `com/yourname/expensetracker/data/ai/worker/DailyBriefingWorker.kt:56-60` | Medium | Cancellation | `catch (e: Exception)` also catches `CancellationException`, so worker cancellation is swallowed and reported as success. | D | CONFIRMED | Re-throw `CancellationException` before the generic exception handler returns `Result.success()`. |
