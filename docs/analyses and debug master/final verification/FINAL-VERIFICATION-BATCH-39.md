...
| 8 | `domain/export/AccountingExporters.kt:12,49,100` | Medium | Concurrency | All three exporters keep `SimpleDateFormat` as instance state, and Hilt provides them as singletons. Concurrent exports can race on shared formatter state. | B | DOWNGRADED | Use `java.time`, `ThreadLocal`, or instantiate the formatter inside each export call. **[RESOLVED BY A.8]** |
...
| 15 | `domain/debug/ServiceDiagnostics.kt:24-41` | Medium | Concurrency | The diagnostic counters use unsynchronized read-modify-write operations on `SharedPreferences`, so concurrent lifecycle callbacks can lose increments. | B | CONFIRMED | Guard updates with a lock or move counters to an atomic/transactional store. **[RESOLVED BY A.8]** |
