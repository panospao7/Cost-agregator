...
> **[RESOLVED BY B.4]** All B.4-scope issues from this batch have been addressed. See `docs/reviews/REVIEW-B4.md` for targeted validation evidence and waivers.
...
## Key Patterns
- At the time of verification, transaction boundaries were too narrow around group writes: validation was happening before the transaction instead of inside it. Resolved by B.4 — Batch 2 (see ISSUE-B4-2).
- The migration chain contained repaired historical schema defects that were not covered by targeted regression tests (`42→43`, `49→50`, `49→50→51→52`, `68→69`). Documented as historical findings; the active migration path (v69→v78) is covered by B.4 migrations and migration tests.
- At the time of verification, batch-plan traceability was poor for this area because the documented file split no longer matched the real implementation layout (six plan-referenced files absent from the repo). Resolved by B.4 — documented as acceptable given B.4's consolidated approach; see Cross-Component Pipeline Issue #4 above and REVIEW-B4.md.
- At the time of verification, existing test coverage was skewed toward `49→50`, `69→70`, and `TransactionType` conversion; the riskiest older migration edge cases and coordinator race paths were untested. B.4 extended coverage via `DatabaseMigrationTest` and `MigrationContractTest` for v76→v77 and v77→v78; older historical paths remain waived per ISSUE-B4-WAIVER.
