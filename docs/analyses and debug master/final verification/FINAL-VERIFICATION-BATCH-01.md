...
| 9a | `AdvancedAnalyticsDashboard.kt:160-206` | High | Boundary | `getMonthlyTrend()` used a broken exclusive end boundary (`23:59:59`) for an end-exclusive month query, which could drop end-of-month transactions. | B | CONFIRMED | Use start-of-next-month as the exclusive end boundary. **[RESOLVED BY A.5]** |
| 9b | `AdvancedAnalyticsDashboard.kt:160-206` | High | Performance / semantics | `getMonthlyTrend()` still performs one repository query per month and omits incoming transfers from income even though the dashboard header counts them. | B | CONFIRMED | Query the full range once and align income rules with `generateDashboardData()`. |
...
| 29 | `SpendingThresholdCalculator.kt:43,83-115,129-133` | Medium | Concurrency | The singleton cache is a plain mutable map accessed from concurrent coroutines on the IO dispatcher. | B | CONFIRMED | Protect cache access with a `Mutex` or replace it with a `ConcurrentHashMap` plus atomic update logic. **[RESOLVED BY A.8]** |
