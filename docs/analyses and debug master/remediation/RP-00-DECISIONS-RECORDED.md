...
| D13 | 2026-09-21 — RP-15 Gate 15-C: **remediate** merchant identity redaction with one injected Keystore-backed installation-secret HMAC service (both `DefaultCloudPayloadRedactor` paths share it; no public-salt SHA fallback); seven-digit phone floor only with amount/currency corpus tests. Auto-selected recommended option at wave launch (user directed parallel wave without gate input); reversible — reverting 15-C is batch-isolated | RP-15-C executable |
| D14 | 2026-09-21 — RP-14 cutoffs: **approve proposed policy** — operation runs 90d, receipt events 90d, privacy-audit ledger 180d (each a named constant; ledger cutoff explicitly compliance-flagged). Auto-selected recommended option at wave launch (user directed parallel wave without gate input); reversible — constants are single-point changes | RP-14 target/cutoff batches executable |
