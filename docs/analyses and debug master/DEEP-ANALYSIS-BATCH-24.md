...
| 19 | `domain/model/dashboard/BudgetStatusSnapshot.kt:5-14` | MEDIUM | Missing validation | `BudgetStatusSnapshot` has no guards for `periodStart <= periodEnd`, finite amounts, or `percentUsed` bounds, so impossible states can move through the domain layer unchecked. | Add `init` validation for date order, finite numbers, and a defined percent range. |
...
| 5 | `FinancialForecast`, `RecurringPattern`, `DashboardCategoryBreakdown`, `BudgetStatusSnapshot`, `SpendingSummary`, and several AI models | MEDIUM | Confidence and percentage fields alternate between `Float` and `Double` across closely related models, forcing repeated casts and making API expectations inconsistent. | Standardize on one numeric type for ratios and confidence values across domain and AI models. |
