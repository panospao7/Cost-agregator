...
class SnoozeReminderActionWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        if (deliveryId == -1L) {
            Timber.w("SnoozeReminderActionWorker: missing deliveryId")
        ...
        val result = executionGuard.runGuardedWithContext(
            ...
            ctx.checkpoint("reminderAction:snooze:beforeLoad")
            coordinator.snoozeReminderDelivery(deliveryId)
            ctx.checkpoint("reminderAction:snooze:afterWrite")
        }
    ...
    companion object {
        const val WORK_NAME = "reminder_action_snooze"
    }
}
