...
/**
 * Tests for [SnoozeReminderActionWorker] — the worker that performs
 * the actual snooze under [WorkerExecutionGuard], replacing direct
 * receiver-side coordinator calls.
 */
class SnoozeReminderActionWorkerTest {

    @Test
    fun `snooze_action_worker_runs_under_guard_and_calls_coordinator`() = runBlocking {
        ...
        val worker = SnoozeReminderActionWorker(
        ...
        coVerify { coordinator.snoozeReminderDelivery(42L) }
    }

    @Test
    fun `snooze_action_worker_missing_delivery_id_returns_failure`() = runBlocking {
        val context = mockk<Context>(relaxed = true)
        val params = mockk<WorkerParameters>(relaxed = true)
        val coordinator = mockk<RecurringLifecycleCoordinator>(relaxed = true)
        val executionGuard = mockk<WorkerExecutionGuard>(relaxed = true)
